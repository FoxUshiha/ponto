...
if (interaction.isModalSubmit() && interaction.customId === 'subscribe_modal') {
  ...
  try {
    ...
    if (status === 'true') {
      // **acrescenta** 30 dias ao tempo atual, em vez de resetar
      await setServerAuth(interaction.guildId, '+30d');
      return interaction.editReply('✅ Pagamento aprovado! +30 dias adicionados à licença.');
    }
...
case 'fechar': {
  ...
  // 1) tenta fechar no servidor atual
  let sourceGuild = guildId;
  let openTs      = await getOpenPoint(guildId, user.id);

  // 2) se não tiver aberto aqui, busca em qualquer servidor
  if (!openTs) {
    const any = await getAnyOpenPoint(user.id);  // :contentReference[oaicite:0]{index=0}
    if (!any) {
      return interaction.editReply('❌ Sem ponto aberto.');
    }
    sourceGuild = any.guild_id;
  }

  // 3) executa o fechamento no servidor correto
  const duration = await endPoint(sourceGuild, user.id); // :contentReference[oaicite:1]{index=1}
  if (!duration) {
    // caso algo tenha falhado internamente
    return interaction.editReply('❌ Sem ponto aberto.');
  }

  // 4) responde de forma condicional
  if (sourceGuild === guildId) {
    return interaction.editReply(`⏱️ Ponto fechado! Duração: ${duration}`);
  } else {
    return interaction.editReply(`⏱️ Ponto fechado de **${sourceGuild}**! Duração: ${duration}`);
  }
}
        case 'ver':
          await interaction.deferReply({ ephemeral: true });
